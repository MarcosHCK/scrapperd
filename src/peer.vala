      ...
      public async bool join (Key to, GLib.Cancellable? cancellable = null) throws GLib.Error
        {
          ...
          try { peers = yield find_peer (to, id, cancellable); } catch (GLib.Error e)
            {
              if (!e.matches (PeerError.quark (), PeerError.UNREACHABLE))

                throw (owned) e;
              else
                {
                  lock (buckets) buckets.drop (to);
                  return false;
                }
            }
      ...
      async bool notify_join (Key[] peers, GLib.Cancellable? cancellable = null) throws GLib.Error
          ...
          requires (peers.length > 0)
        {
          ...
          for (unowned uint i = 0; i < ALPHA; ++i)
            {
              dones [i] = 0;
              lists [i] = new GenericArray<Key> (1 + peers.length / ALPHA);
            }
